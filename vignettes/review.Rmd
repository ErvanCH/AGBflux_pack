...
* Grouping: Consider grouping functions in files by topic.
* Tab versus space: Avoid using tab to insert spaces. Instead use space.
* Lines width: Keep the widh of each line no longer than 80 characters. This improves readability. To achive this:

  * Guide your eyes setting a margin column marker: Tools > Global Options > Code > Display > Show Margin > Margin Column = 80.
  
  * For comments (both normal and roxygen comments), master the keyboard shortcuts Shift + Control + C to comment a line/paragraph, and Shift + Control + / to brake a paragraph into lines no longer than 80 characters.

* Use a style guide (see section Style) and reformat the code with (select then) Control + Shift + A, or  `formatR::tidy_code()`.
